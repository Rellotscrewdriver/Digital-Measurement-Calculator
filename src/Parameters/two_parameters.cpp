...
void Parameter::two_parameters(){
    ...
    H_Exp.check_if_user_entered_string_and_shape_3D(is_3D);
        ...
        if (H_Exp.check_two_conditions(first_parameter, second_parameter)){
            ...
            Triangle trio;
            cout << color::yellow << dis.output << trio.ATriangle(first_parameter, second_parameter) << color::reset << endl;
        } else if(Shape_Value == 18){
            Triangle trio;
            cout << color::yellow << dis.output << trio.AofITriangle(first_parameter, second_parameter) << color::reset << endl;
        } else {}
