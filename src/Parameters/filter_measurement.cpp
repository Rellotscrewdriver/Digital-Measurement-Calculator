...
Log.open("Log.txt", ios::app);
    if(Measure_Type == 1){
        ...
        << color::yellow << ANSWER << Area << color::reset << NEWLINE;
        Log << "\n\nThe Answer was " << Area << endl;
        hr.horizontal_line(54, "=");
	} else if(Measure_Type == 2){
        cout << EQUAL << Perimeter << NEWLINE 
        << color::yellow << ANSWER << Perimeter << color::reset << NEWLINE;
...
Log.open("Log.txt", ios::app);
	...
	if(Measure_Type == 1){
        ...
        << color::yellow << ANSWER << Surface_Area << color::reset << NEWLINE;
        Log << "\n\nThe Answer was " << Surface_Area << endl;
        hr.horizontal_line(54, "=");
        Log.close();
	} else if(Measure_Type == 2){
        cout << EQUAL << Volume << NEWLINE
        << color::yellow << ANSWER << Volume << color::reset << NEWLINE;
...
Log.open("Log.txt", ios::app);
	...
	if(Measure_Type == 1){
		...
		<< color::yellow << ANSWER << Curved_Surface_Area << color::reset << NEWLINE;
        Log << "\n\nThe Answer was " << Curved_Surface_Area << endl;
        hr.horizontal_line(54, "=");
        Log.close();
	} else if(Measure_Type == 2){
	    cout << EQUAL << Total_Surface_Area << color::reset << NEWLINE
		<< color::yellow << ANSWER << Total_Surface_Area << color::reset << NEWLINE;
